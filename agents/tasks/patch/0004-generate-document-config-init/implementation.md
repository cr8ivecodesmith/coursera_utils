...
- Create: `src/study_utils/generate_document/__init__.py`, `src/study_utils/generate_document/cli.py`, `src/study_utils/generate_document/config.py`, `src/study_utils/generate_document/template.toml`, potentially `src/study_utils/generate_document/runner.py` (or similar) for the core generation orchestration.
...
## Solution Plan
- Architecture/pattern choices: follow convert-markdown’s package layout to keep CLI, config resolution, and execution seams isolated, aligning with the seam-first guidance in `patterns-and-architecture.md`; leverage `importlib.resources` for template access and shared workspace helpers for default paths.
...
- Stepwise checklist:
  ...
  - [ ] Wire `cli.py` to provide both the document generation command and a nested `config init` subcommand mirroring convert-markdown options.
  - [ ] Register the bundled `template.toml` in `core.config_templates` and move existing TOML content into the new resource.
  ...
  - [ ] Expand/adjust tests: unit tests for config resolution precedence and error messaging, CLI tests for the new command group, and cleanup of obsolete shim fixtures.
